...
namespace sdds
{
	...
	ifstream& Item::load(std::ifstream& ifstr) {
		...
		ifstr.getline(insertDescription, 1000, '\t');
		...
		if (!ifstr) {
			...
			m_description = new char[strlen(insertDescription) + 1 ];
			strcpy(m_description, insertDescription);
